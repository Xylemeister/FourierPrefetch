...
public:
    ...
    void insert(uint64_t region_number, uint64_t pc, int offset) {
        ...
        Super::insert(key, {region_number, pc, offset});
        Super::rp_promote(key);
    }
...
private:
    ...
    uint64_t build_key(uint64_t pc, uint64_t address) {
        ...
        address &= (1 << this->addr_width) - 1; /* use [addr_width] bits from address */
        uint64_t key = (pc << this->addr_width) | address;
...
class SMS {
public:
    SMS(int pattern_len, int addr_width, int pc_width, int pattern_history_table_size,
...
public:
    std::vector<bool> find_in_pht(uint64_t pc, uint64_t address) {
        if (this->debug_level >= 1) {
            std::cerr << "[SMS] find_in_pht(pc=" << pc << ", address=" << address << ")" << std::endl;
        }

        this->pht_lookup_cnt += 1;
        std::vector<bool> pattern = this->pht.find(pc, address);
        if (!pattern.empty())
